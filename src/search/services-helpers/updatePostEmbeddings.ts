    ...
    try {
      ...
      const embeddingPosts = batch.reduce((acc, post, index) => {
        ...
        if (description.status === 'fulfilled' && description.value) {
          ...
          Logger.error(`Failed to create summary for post ${post.id}: ${description.reason}`);
        }
        return acc;
      ...
      // 3. Tạo embeddings
      const embeddings = embeddingPosts.length > 1 ? await OpenAIService.createEmbeddingBatch(
      ...
      await Logger.error(`Batch update failed for posts ${batch.map(p => p.id).join(',')}: ${error}`);
    }
